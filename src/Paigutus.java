...
public class Paigutus {
	...
	static ArrayList<Integer> laevadMeres = new ArrayList<>();
	...
	static boolean vabaMeri(ArrayList<Integer> laev) {
		...
		for (int i = 0; i < laev.size(); i++) {
			if (merepiir.contains(laev.get(i)) == true || laevadMeres.contains(laev.get(i)) == true) {
	...
	static ArrayList<Integer> looLaev(boolean vertikaalne, int pikkus) {
	...
	public static void main(String[] args) {
		...
		for (int i = 4; i > 0; i--){
			for (int j = 0; j < n; j++) {
				looLaev(Math.random() < 0.49, i); // Laeva suund: boolean vertikaalne
			}
			n++;
		}
		...
		for (int i = 0; i < laevadMeres.size(); i++) {
			mängulaud[laevadMeres.get(i)]++;
		}
